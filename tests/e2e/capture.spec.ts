...
test("lets a user choose a visible image from an ordinary page", async ({ page }, testInfo) => {
  ...
  await page.screenshot({ path: testInfo.outputPath("capture-picker-desktop.png") });
...
test("keeps candidate controls visible at phone width", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("button", { name: "Save image candidate 1" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Save image candidate 2" })).toBeVisible();
  await page.screenshot({ path: testInfo.outputPath("capture-picker-phone.png") });
});
