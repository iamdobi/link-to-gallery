...
export function FolderPickerSheet({ open, folders, action, onClose, onConfirm }: FolderPickerSheetProps) {
  ...
  return (
    <Sheet onClose={onClose} open={open} title={action === "folder_add" ? "Add folders" : "Remove folders"}>
      ...
      <button className="mt-6 min-h-11 w-full bg-slate-900 px-4 text-sm font-medium text-white disabled:opacity-50" disabled={!selectedIds.size} onClick={() => onConfirm([...selectedIds])} type="button">{action === "folder_add" ? "Add to selected images" : "Remove from selected images"}</button>
    </Sheet>
  );
}
